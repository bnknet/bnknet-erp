        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {[
            ...
            <button key={p.label} onClick={() => { setDateFrom(p.from); setDateTo(p.to); }}
              className="px-3 py-2 rounded-xl text-sm font-medium bg-white border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors">
              {p.label}
            </button>
          ))}
                  ...
                  <div key={log.id} onClick={() => openDetail(log)}
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <span className="text-sm font-bold text-gray-800">{log.author_name}</span>
                        <span className="text-xs px-2 py-0.5 rounded-md bg-slate-100 text-slate-500">{log.company}</span>
                      </div>
                    </div>
                    <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-3">
                      <div>
                        <div className="text-xs text-blue-400 font-medium mb-1">진행업무</div>
                        <p className="text-sm text-gray-700 line-clamp-2 whitespace-pre-line">{log.in_progress || '-'}</p>
                      </div>
                      <div>
                        <div className="text-xs text-orange-400 font-medium mb-1">예정업무</div>
                        <p className="text-sm text-gray-700 line-clamp-2 whitespace-pre-line">{log.planned || '-'}</p>
                      </div>
                      <div>
                        <div className="text-xs text-purple-400 font-medium mb-1">특이사항</div>
                        <p className="text-sm text-gray-700 line-clamp-2 whitespace-pre-line">{log.notes || '-'}</p>
                      </div>
                    </div>
                  </div>
                ))}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <div className="flex items-start justify-between mb-6">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs px-2 py-0.5 rounded-md bg-slate-100 text-slate-500">{selected.company}</span>
            </div>
          ...
          {canEdit(selected) && (
            <div className="flex gap-2">
              <button onClick={() => openForm(selected)}
                className="px-3 py-1.5 text-sm text-blue-600 border border-blue-200 rounded-lg hover:bg-blue-50">수정</button>
              <button onClick={() => handleDelete(selected.id)}
                className="px-3 py-1.5 text-sm text-red-500 border border-red-200 rounded-lg hover:bg-red-50">삭제</button>
            </div>
          )}
        </div>
        ...
        <div className="space-y-5">
          {[
            ...
            <div key={label}>
              <div className={`text-xs font-semibold mb-2 text-${color}-500`}>{label}</div>
              <div className="bg-gray-50 rounded-xl px-4 py-3 text-sm text-gray-700 whitespace-pre-line min-h-[60px]">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        ...
        <div className="space-y-4">
          {[
            ...
            <div key={key}>
              <label className={`block text-sm font-medium text-${color}-500 mb-1.5`}>{label}</label>
              <textarea
