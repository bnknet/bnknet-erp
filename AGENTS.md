# AGENTS.md — BNKNET ERP

AI 코딩 에이전트(Claude Code·Copilot·Codex 등)가 이 레포에서 작업할 때 따르는 공통 규칙.
(Claude Code는 CLAUDE.md → 이 파일을 자동으로 불러온다. 규칙은 여기 한 곳에서만 관리한다.)
...
- **매칭엔 항상 옵션 반영**: 수집상품명→대표상품명 매칭은 반드시 `repNameFor(collectName, collectOption)`로 한다. 색상(자연갈색/흑색) 등은 상품명에 없고 **수집옵션에만** 있을 수 있어, `matchProduct`를 옵션 없이 직접 부르면 하드코딩 기본값(예: 흑색)으로 오차감·오집계된다. 재고 차감·매출/원가 집계·출고 집계 등 모든 매칭 지점에서 조회 SQL에 `collect_option`을 포함하고 함께 넘길 것.
...
- **데이터 손실 방지**: 삭제-후-삽입 대신 **삽입 성공 확인 후 삭제**. 저장 실패는 조용히 넘기지 말고 사용자에게 알린다.
